...
#define ABDL_DIST(m, n) BLKADDR_AFFINITY(A, double, m, n)
#define ABDL ABDL_DIST

void CORE_dtile_zero_affinity_quark(Quark* quark);

/***************************************************************************/
/**
 *  Conversion from LAPACK F77 matrix layout to tile layout - static scheduling
 **/
void plasma_pdlapack_to_tile_affinity(plasma_context_t *plasma)
{
    double *Af77;
    int lda;
    PLASMA_desc A;
 ...
 **/
void plasma_pdlapack_to_tile_affinity_quark(double *Af77, int lda, PLASMA_desc A,
 ...
 *  Conversion from LAPACK F77 matrix layout to tile layout - static scheduling
 **/
void plasma_pdtile_to_lapack_affinity(plasma_context_t *plasma)
{
    PLASMA_desc A;
    double *Af77;
    int lda;
 ...
 *  Conversion from LAPACK F77 matrix layout to tile layout - dynamic scheduling
 **/
void plasma_pdtile_to_lapack_affinity_quark(PLASMA_desc A, double *Af77, int lda,
                                   PLASMA_sequence *sequence, PLASMA_request *request)
{
    double *f77;
    double *bdl;
 ...
 **/
void plasma_pdtile_zero_affinity(plasma_context_t *plasma)
{
    PLASMA_desc A;
    PLASMA_sequence *sequence;
    PLASMA_request *request;
 ...
 **/
void plasma_pdtile_zero_affinity_quark(PLASMA_desc A, PLASMA_sequence *sequence, PLASMA_request *request)
{
    double *bdl;
    plasma_context_t *plasma;
    int X1, Y1;
    ...
    for (m = 0; m < A.mt; m++)
    {
        ...
        for (n = 0; n < A.nt; n++)
        {
            ...
            bdl = ABDL(m, n);
            QUARK_Insert_Task(plasma->quark, CORE_dtile_zero_affinity_quark, &task_flags,
                sizeof(int),                       &X1,  VALUE,
                sizeof(int),                       &X2,  VALUE,
                sizeof(int),                       &Y1,  VALUE,
                sizeof(int),                       &Y2,  VALUE,
