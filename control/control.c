...
int PLASMA_Init_Affinity(int cores, int *coresbind)
{
    ...
    CHECK_CUDA(cudaStreamCreateWithFlags(&plasma->cuda_stream[0], cudaStreamNonBlocking));
    CHECK_CUBLAS(cublasCreate(&plasma->cublas_handle[0]));
    CHECK_CUBLAS(cublasSetStream(plasma->cublas_handle[0], plasma->cuda_stream[0]));
    CHECK_CUSOLVER(cusolverDnCreate(&plasma->cusolver_handle[0]));
    CHECK_CUSOLVER(cusolverDnCreateParams(&plasma->cusolver_params[0]));
    CHECK_CUSOLVER(cusolverDnSetStream(plasma->cusolver_handle[0], plasma->cuda_stream[0]));
#endif

    for (core = 1; core < plasma->world_size; core++) {
#ifdef PLASMA_WITH_CUDA
        CHECK_CUDA(cudaStreamCreateWithFlags(&plasma->cuda_stream[core], cudaStreamNonBlocking));
        CHECK_CUBLAS(cublasCreate(&plasma->cublas_handle[core]));
        CHECK_CUBLAS(cublasSetStream(plasma->cublas_handle[core], plasma->cuda_stream[core]));
        ...
        CHECK_CUSOLVER(cusolverDnSetStream(plasma->cusolver_handle[core], plasma->cuda_stream[core]));
#endif
