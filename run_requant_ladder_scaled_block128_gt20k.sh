#!/usr/bin/env bash
# Full-ladder MX sweep with block (subtile 128x128) MX storage.
...
SWEEP_DIR="$OUT_DIR/requant_ladder_scaled_block128_gt20k"
RUN_LOG_DIR="$SWEEP_DIR/run_logs"
SUMMARY_FILE=${SUMMARY_FILE:-"$SWEEP_DIR/summary_requant_ladder_scaled_block128_gt20k.tsv"}
MASTER_LOG="$SWEEP_DIR/sweep_master.log"
mkdir -p "$RUN_LOG_DIR"

{
  echo "[START] $(date -Is)"
  echo "[CFG] sweep=requant_ladder_scaled_block128_gt20k"
