#!/usr/bin/env bash
# MX-staircase ladder sweep — tile granularity (single shared scale per 2048×2048 tile).
# Ladder: e2m1 (MXFP4) -> mx_e4m3 (MXFP8) -> fp16 (plain IEEE) -> fp32 -> fp64.
# Same configuration as run_mx_staircase_vec1d32_gt20k.sh but with MX_MX_MODE=tile.
# Sizes: {32k, 40k, 65k} (gt20k); ε: {1e-8 .. 1e-5}; both fz and gu underflow modes.
#
...
OUT_DIR=${OUT_DIR:-"$SCRIPT_DIR/mx_staircase_tile_gt20k"}
...

{
  ...
  echo "[CFG] mx_mode=tile  (one shared E8M0 scale per nb x nb tile)"
...
run_one() {
  ...
  local sweep="mx_staircase_tile_${underflow}"
  ...
  echo "$sweep,$bin,$n,$nb,tile,$underflow,$eps,${rel:-NA},${abs:-NA},${res:-NA},\"$tb\"" >> "$CSV"
}
