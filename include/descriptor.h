...
inline static void *plasma_getaddr_affinity(PLASMA_desc A, int m, int n) {
    const int nt_each_each_processors = A.mt / A.uniqueDevice;

    size_t mm = m + A.i / A.mb;
    int deviceIdx;

    if (mm >= nt_each_each_processors * A.uniqueDevice) {
        deviceIdx = A.uniqueDevice - 1;
    } else {
        deviceIdx = mm / nt_each_each_processors;
    }

    size_t mmOfDevice = mm - deviceIdx * nt_each_each_processors;
    size_t nn = n + A.j / A.nb;
    size_t eltsize = plasma_element_size(A.dtyp);
    size_t offset = 0;

    void *ptr = ((void **)A.mat)[deviceIdx];
    return (void *)((char *)ptr + (nn * (mm < A.lm1 ? A.mb : A.lm % A.mb) * A.nb +
                                   mmOfDevice * A.mb * A.ln) *
                                      eltsize);
}
