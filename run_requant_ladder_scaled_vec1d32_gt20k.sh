#!/usr/bin/env bash
# Full-ladder MX sweep with canonical vec1D MX storage (32-element groups, E8M0 shared scale).
# Selection criteria: bound, ladder mode = full.
# Hardcoded ladder steps: e2m1 (MXFP4) -> mx_e4m3 (MXFP8) -> mx_fp16 (MXFP16) -> fp32 -> fp64 fallback.
...
SWEEP_DIR="$OUT_DIR/requant_ladder_scaled_vec1d32_gt20k"
RUN_LOG_DIR="$SWEEP_DIR/run_logs"
SUMMARY_FILE=${SUMMARY_FILE:-"$SWEEP_DIR/summary_requant_ladder_scaled_vec1d32_gt20k.tsv"}
MASTER_LOG="$SWEEP_DIR/sweep_master.log"
mkdir -p "$RUN_LOG_DIR"

{
  echo "[START] $(date -Is)"
  echo "[CFG] sweep=requant_ladder_scaled_vec1d32_gt20k"
  echo "[CFG] bins=$BIN_LIST"
  echo "[CFG] eps=$EPS_LIST"
  echo "[CFG] selection=bound ladder=full (e2m1 -> mx_e4m3 -> mx_fp16 -> fp32 -> fp64)"
  echo "[CFG] mx_mode=vec1d subtile=32 (canonical MX 1D, 32-element groups)"
