#!/usr/bin/env bash
# Baseline plain-FP8 (E4M3) sweep, no MX block scaling.
# Uses the LEGACY epsilon-cutoff selector (NOT the bound ladder system):
# decides format per tile from epsilonRatio vs sourceEpsilon thresholds.
#   epsilonRatio > eps/fp32_eps  -> FP64
#   epsilonRatio > eps/fp16_eps  -> FP32 bucket
#   epsilonRatio > lowCutoff     -> FP16 bucket
#   otherwise                    -> FP8_E4M3 (FORMAT)
...
SWEEP_DIR="$OUT_DIR/requant_baseline_fp8_gt20k"
RUN_LOG_DIR="$SWEEP_DIR/run_logs"
SUMMARY_FILE=${SUMMARY_FILE:-"$SWEEP_DIR/summary_requant_baseline_fp8_gt20k.tsv"}
MASTER_LOG="$SWEEP_DIR/sweep_master.log"
mkdir -p "$RUN_LOG_DIR"

{
  echo "[START] $(date -Is)"
  echo "[CFG] sweep=requant_baseline_fp8_gt20k"
...
for BIN_PATH in $BIN_LIST; do
  ...
  for eps in "${EPS_ARR[@]}"; do
    ...
    # Legacy epsilon-cutoff selector: do NOT set MX_SELECTION_CRITERIA or MX_BOUND_LADDER.
    RUN_ENV=(
      MX_SKIP_KL=1
      BUILD="$BUILD_VAL"
      MX_MX_MODE=tile
